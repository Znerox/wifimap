...
if ($handle) {
  while (($line = fgets($handle)) !== false) {
    ...
    if  (substr($line, 3, 1) == ":") {
      ...
      $router_bssid = substr($router_data_array[0], 1, -1);
      $router_ssid = substr($router_data_array[1], 1, -1);
      $router_frequency = substr($router_data_array[2], 1, -1);
      $router_capabilities = substr($router_data_array[3], 1, -1);
      $router_lasttime = substr($router_data_array[4], 1, -1);
      $router_lastlat = substr($router_data_array[5], 1, -1);
      $router_lastlon = substr($router_data_array[6], 1, -1);
      $router_bestlevel = substr($router_data_array[8], 1, -1);
      $router_bestlat = substr($router_data_array[9], 1, -1);
      $router_bestlon = substr($router_data_array[10], 1, 11);
