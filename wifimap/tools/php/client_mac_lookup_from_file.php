...
// Do an initial database search for clients with missing vendor
...
if ($clientsWithNullVendor > 0) {
  ...
  echo $clientsWithNullVendor . " devices left without vendor (before the round just completed)<br>";
  ...
  while ($stopLoop == 0) {
    ...
    // Find clients in the database without vendor information
    ...
    if(preg_match($pattern, $file_contents, $match)){
      ...
      // Update database with found vendor, for all clients containing this OUI
      $query2 = 'UPDATE clients SET vendor="' . $vendor_name . '" WHERE client_mac LIKE "' . $mac_from_db_trimmed . ':__:__:__";';
    ...
    // Check if there are still clients in database with missing vendor information
    ...
    // Stop the loop if there are no clients in database with missing vendor information
    if ($clientsWithNullVendor == 0) {
      $stopLoop = 1;
