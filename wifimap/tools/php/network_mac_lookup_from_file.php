...
// Do an initial database search for networks with missing vendor
...
if ($networksWithNullVendor > 0) {
  ...
  echo $networksWithNullVendor . " devices left without vendor (before the round just completed)<br>";
  ...
  while ($stopLoop == 0) {
    ...
    // Find networks in the database without vendor information
    ...
    if(preg_match($pattern, $file_contents, $match)){
      ...
      // Update database with found vendor, for all networks containing this OUI
      $query2 = 'UPDATE network SET vendor="' . $vendor_name . '" WHERE bssid LIKE "' . $mac_from_db_trimmed . ':__:__:__";';
    ...
    // Check if there are still networks in database with missing vendor information
    ...
    // Stop the loop if there are no networks in database with missing vendor information
    if ($networksWithNullVendor == 0) {
      $stopLoop = 1;
